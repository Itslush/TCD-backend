...
def increment_fling_count():
    ...
    logging.info(f"Fling reported by '{bot_name}' on {server_id} for '{target_name}'. New total: {current_total_count}. Added to recent list.")
...
def get_stats_data():
    stats_start_time = time.time(); bot_names = set(); server_count = 0; bots_by_region = defaultdict(set); now = time.time(); current_total_flings = 0
    logging.debug("Calculating stats for / endpoint.")
    with data_lock:
        reservations = shared_data.get("serverReservations", {}); valid_reservations = {sid: res for sid, res in reservations.items() if not is_reservation_stale(res)[0]}
        for server_id, res in valid_reservations.items():
            bot_name = res.get("botName", "UnknownBot"); region = res.get("region", "UnknownRegion"); bot_names.add(bot_name); bots_by_region[region].add(bot_name)
        server_count = len(valid_reservations); bot_count = len(bot_names); regional_bot_counts = {region: len(bots) for region, bots in bots_by_region.items()}
        current_total_flings = total_flings_reported
        last_calc = shared_data["last_stats_calc"]; time_diff = now - last_calc["time"]; fling_diff = current_total_flings - last_calc["fling_count"]
        fling_rate_per_minute = 0.0
        if last_calc["time"] > 0 and time_diff > 1:
            if fling_diff < 0 and abs(fling_diff) > last_calc["fling_count"] * 0.5: logging.warning(f"Potential fling counter reset detected."); fling_diff = 0
            flings_per_second = max(0.0, fling_diff / time_diff); fling_rate_per_minute = flings_per_second * 60
        shared_data["last_stats_calc"]["time"] = now; shared_data["last_stats_calc"]["fling_count"] = current_total_flings
    stats_data = {"botCount": bot_count, "serverCount": server_count, "totalFlings": current_total_flings, "flingRatePerMinute": fling_rate_per_minute, "botsPerRegion": regional_bot_counts}
    stats_end_time = time.time(); logging.debug(f"Stats calc took {stats_end_time - stats_start_time:.4f}s."); return jsonify(stats_data)

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    logging.info(f"Starting TCD-API (Polling Fling Feed) on port {port}...")
    try:
        pass
    except Exception as e:
         logging.critical(f"Failed during startup: {e}", exc_info=True)
         exit(1)
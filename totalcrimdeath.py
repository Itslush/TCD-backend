...
def get_reservations():
    ...
    with data_lock:
        ...
        reservations_list = sorted(valid_reservations.values(), key=lambda x: x.get('timestamp', 0), reverse=True)
    ...
    with data_lock:
        ...
        for s_id, res in reservations.items():
            if res.get("botName") == bot_name and s_id != server_id:
                ...
                break
        if existing_reservation_to_release and existing_reservation_to_release in reservations: del reservations[existing_reservation_to_release]
...
def update_reservation():
    ...
    if new_status is not None and new_status not in ['active', 'flinging']: return jsonify({"error": "Invalid status. Must be 'active' or 'flinging'."}), 400
    with data_lock:
        reservations = shared_data.get("serverReservations", {}); current_reservation = reservations.get(server_id)
        if not current_reservation:
            logging.warning(f"Update: Reservation for {server_id} by {bot_name} not found. Creating.")
            new_reservation = {"serverId": server_id, "botName": bot_name, "timestamp": time.time(), "status": new_status or "active", "region": data.get('region', 'Unknown'), "initialPlayerCount": current_player_count, "currentPlayerCount": current_player_count}
            reservations[server_id] = new_reservation; return jsonify(new_reservation), 201
        if current_reservation.get("botName") != bot_name:
            logging.error(f"Update Auth Fail: {bot_name} tried to update {server_id} owned by {current_reservation.get('botName')}"); return jsonify({"error": "Reservation owned by another bot"}), 403
        updated = False
        if new_status is not None and current_reservation.get('status') != new_status: current_reservation['status'] = new_status; updated = True
        if current_player_count is not None and current_reservation.get('currentPlayerCount') != current_player_count: current_reservation['currentPlayerCount'] = current_player_count; updated = True
...
def release_reservation():
    ...
    with data_lock:
        reservations = shared_data.get("serverReservations", {}); current_reservation = reservations.get(server_id)
        if not current_reservation:
            logging.warning(f"Release Not Found: {server_id} by {bot_name}"); return jsonify({"message": "Reservation not found"}), 200 # OK (idempotent)
        if current_reservation.get("botName") != bot_name:
            logging.error(f"Release Auth Fail: {bot_name} tried to release {server_id} owned by {current_reservation.get('botName')}"); return jsonify({"error": "Reservation owned by another bot"}), 403
        del reservations[server_id]; logging.info(f"Release Success: {bot_name} released {server_id}"); return jsonify({"message": "Reservation released"}), 200
